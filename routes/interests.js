...
router.post('/send', authMiddleware, async (req, res) => {
  try {
    ...
    logger.error('Send interest error:', err);
...
router.post('/respond', authMiddleware, async (req, res) => {
  try {
    ...
    // Mark the original 'interest' notification as resolved
    await supabase
      .from('notifications')
    ...
    if (action === 'accept') {
      ...
      // Emit chat_created to investor if online
      if (chat) {
        const investorSocketId = await onlineUsers.get(interest.investor_id);
        if (investorSocketId) {
          io.to(investorSocketId).emit('chat_created', {
            ...chat,
            other_user: chat.founder,
          });
          logger.debug(`Emitted chat_created to investor ${interest.investor_id}`);
        }
      }
    ...
    logger.error('Respond interest error:', err);
...
router.get('/received', authMiddleware, async (req, res) => {
  try {
    ...
    logger.error('Get received interests error:', err);
