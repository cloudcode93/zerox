...
const { getNotificationQueue } = require('../queues/queues');
 ...
 * The notification worker will handle FCM sending with retries.
 */
async function queuePushNotification(userId, title, body, data = {}) {
  try {
    ...
    await queue.add('send-push', {
      ...
      jobId,  // Prevents duplicate jobs for the same notification
    });

    logger.debug(`[Notif] Queued push for user ${userId}: ${title}`);
  } catch (err) {
    logger.error('[Notif] Failed to queue push notification:', err.message);
    // Fail silently — notification won't be sent but app continues
  }
}
